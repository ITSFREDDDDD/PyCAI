...
#        chat.new_chat('CHAR', 'CHAT_ID', 'CREATOR_ID') may have made a mistake here, it looks like there's a new chat function separate from new message
...
#+BEGIN_SRC python :tangle "sample.py" :noweb yes
  class chat:
      ...
      <<get_history.chat>> # Gets a specific history from histories json object
